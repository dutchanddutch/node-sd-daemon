...
const { AF_UNIX, SOCK_DGRAM,
	socket, sendto, close,
	getsockopt_int, setsockopt_int, SOL_SOCKET, SO_SNDBUF,
	cmsg_rights, errnoException } = require('./socket-calls');

const address = '/run/systemd/journal/socket';
const fd = socket( AF_UNIX, SOCK_DGRAM, 0 );
if( fd < 0 )
	throw errnoException( fd, 'socket(AF_UNIX, SOCK_DGRAM, 0)' );
try {
	const buflen = 8 * 1024 * 1024;
	if( getsockopt_int( fd, SOL_SOCKET, SO_SNDBUF ) < buflen )
		setsockopt_int( fd, SOL_SOCKET, SO_SNDBUF, buflen );
} catch( err ) {
}
...
const bin_encode = function( fields ) {
	...
	for( let [field, value] of fields ) {
		...
		if( value.length > MAX_INLINE || value.includes( newline ) ) {
