...
const { AF_UNIX, SOCK_DGRAM,
	socket, sendto, close,
	getsockopt, setsockopt, SOL_SOCKET, SO_SNDBUF,
	...
	throw errnoException( fd, 'socket(AF_UNIX, SOCK_DGRAM, 0)' );
{
	...
	let [ err, bufsize ] = getsockopt( fd, SOL_SOCKET, SO_SNDBUF )
	if( err < 0 )
		throw errnoException( fd, 'getsockopt( fd, SOL_SOCKET, SO_SNDBUF )' );
	if( bufsize < reqbufsize ) {
		err = setsockopt( fd, SOL_SOCKET, SO_SNDBUF, reqbufsize );
		if( err < 0 )
			throw errnoException( fd, `setsockopt( fd, SOL_SOCKET, SO_SNDBUF, ${reqbufsize} )` );
	}
}

const send_raw = function( data ) {
	let r = sendto( fd, address, data );
	if( r >= 0 )
		return true;
...
const is_iterable = function( value ) {
...
const bin_encode = function( fields ) {
