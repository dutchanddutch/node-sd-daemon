...
const stderr_is_journal = () => {
	let m = /^(\d+):(\d+)$/.exec( process.env.JOURNAL_STREAM ?? '' );
	if( ! m )
		return false;
	let { dev, ino } = require('fs').fstatSync( 2 );
	return +m[1] === dev && +m[2] === ino;
};

journal.is_stderr = stderr_is_journal();
